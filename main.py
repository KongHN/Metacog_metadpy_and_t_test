...
def calculate_metrics(file_path, rating_type, n_ratings=None):
    ...
    for subj_idx, group in tqdm(subj_groups, desc="Processing subjects", unit="subject"):
        ...
        results.append({
            ...
            'meta_d': meta_d,
            'm_ratio': m_ratio,
            'm_diff': m_diff,  # 新增：添加 m_diff 到结果字典
            'Block': block_count,
            'Trials': trials_count
        })
...
merged_df = pd.merge(result_df, xlsx_df[['Name_in_database', 'Category']],
                     left_on='Exp_Name', right_on='Name_in_database', how='left')

# 删除多余的列
merged_df = merged_df.drop(columns=['Name_in_database'])

# 调整列顺序
column_order = ['Category', 'Exp_Name', 'Subj_idx', 'dprime', 'meta_d', 'm_ratio', 'm_diff', 'Block', 'Trials', 'Part']
