...
def calculate_metrics(file_path, rating_type, n_ratings=None):
    ...
    for subj_idx, group in data.groupby('Subj_idx'):
        ...
        # 计算 D prime
        # 手动计算 hits, misses, fas, crs
        hits = ((group['Stimuli'] == 1) & (group['Responses'] == 1)).sum()
        misses = ((group['Stimuli'] == 1) & (group['Responses'] == 0)).sum()
        fas = ((group['Stimuli'] == 0) & (group['Responses'] == 1)).sum()
        crs = ((group['Stimuli'] == 0) & (group['Responses'] == 0)).sum()

        # 避免除零错误
        if (hits + misses) == 0 or (fas + crs) == 0:
            d_prime = 0
        else:
            hit_rate = hits / (hits + misses)
            fa_rate = fas / (fas + crs)
            d_prime = dprime(hit_rate=hit_rate, fa_rate=fa_rate)

        # 计算 meta d’ 和 m ratio
        fit = metad(
            data=group,
            nRatings=n_ratings if rating_type == 2 else 4,
            stimuli='Stimuli',
            accuracy='Accuracy',
            confidence='Confidence',
            verbose=0
...
# 获取用户输入
file_path = input("请输入本地 CSV 文件的地址：")
rating_type = int(input("请输入评分类型（1 表示连续数据，2 表示离散数据）："))
if rating_type == 2:
    n_ratings = int(input("请输入离散评分的点数："))
else:
    n_ratings = None
...
# 读取 Excel 文件
xlsx_file_path = input("请输入 Excel 文件的地址：")
...
# 调整列顺序
column_order = ['Category', 'Name', 'Subj_idx', 'D prime', 'meta d’', 'm ratio', 'Block', 'Trails', 'Part']
