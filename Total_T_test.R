...
for (exp_name in exp_names) {
  ...
  if (length(first_data_ratio) == length(second_data_ratio)) {
    ...
    # 提取自由度
    df_ratio <- t_test_result_ratio$parameter
    
    # 计算 Cohen's d 效应量
    mean_diff_ratio <- mean(first_data_ratio - second_data_ratio)
    sd_diff_ratio <- sd(first_data_ratio - second_data_ratio)
    cohens_d_ratio <- mean_diff_ratio / sd_diff_ratio
    cohens_d_ratio <- round(cohens_d_ratio, 3)
  ...
  if (length(first_data_diff) == length(second_data_diff)) {
    ...
    # 提取自由度
    df_diff <- t_test_result_diff$parameter
    
    # 计算 Cohen's d 效应量
    mean_diff_diff <- mean(first_data_diff - second_data_diff)
    sd_diff_diff <- sd(first_data_diff - second_data_diff)
    cohens_d_diff <- mean_diff_diff / sd_diff_diff
    cohens_d_diff <- round(cohens_d_diff, 3)
...
# 提取总的 m_ratio 自由度
total_df_ratio <- total_t_test_result_ratio$parameter

# 计算总的 m_ratio Cohen's d 效应量
total_mean_diff_ratio <- mean(total_first_data_ratio - total_second_data_ratio)
total_sd_diff_ratio <- sd(total_first_data_ratio - total_second_data_ratio)
total_cohens_d_ratio <- total_mean_diff_ratio / total_sd_diff_ratio
total_cohens_d_ratio <- round(total_cohens_d_ratio, 3)
...
# 提取总的 m_diff 自由度
total_df_diff <- total_t_test_result_diff$parameter

# 计算总的 m_diff Cohen's d 效应量
total_mean_diff_diff <- mean(total_first_data_diff - total_second_data_diff)
total_sd_diff_diff <- sd(total_first_data_diff - total_second_data_diff)
total_cohens_d_diff <- total_mean_diff_diff / total_sd_diff_diff
total_cohens_d_diff <- round(total_cohens_d_diff, 3)
...
total_results <- data.frame(Exp_Name = c("m_ratio", "m_diff"),
                            ...
                            df = c(total_df_ratio, total_df_diff),
                            cohens_d = c(total_cohens_d_ratio, total_cohens_d_diff))
